...
Page({
  ...
  onLoad(options) {
    ...
    c1.collection('yuanqu').limit(20).get()
    //请求成功
    .then(res =>{
      //console.log(res)
      this.setData( 
...
onDetail: function (e) {
  ...
  wx.navigateTo({
    url: "/packageA/pages/yuanqu_re_detail?id=" + imgId   //?id连字符加上imgId
});
},

onReachBottom: function () {
  if(this.data.page>553)
  {
    wx.showToast({
      title: '无更多数据',
    })
...
c1.collection('yuanqu').skip(page*20).limit(20).get({
  success(res){
